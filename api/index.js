 ...
 * Version: 6.8 (Smart Import & Context Logic Fix)
 */
...
const DEFAULT_CONFIG = {
    ...
    customAppGroups: [], // 用户自定义分流组
    customGlobalRules: [], // 用户自定义全局规则
    groupOrder: [...DEFAULT_APP_NAMES], // 排序
    // DNS 覆写配置
    dnsSettings: {
        enable: true,
        ipv6: false,
...
module.exports = async (req, res) => {
    try {
        ...
        res.send(`<div style="padding:20px;"><h3>🔴 Server Error</h3><p>请检查 KV 数据库连接。</p><pre>${err.stack}</pre></div>`);
    }
};
...
async function generateConfig(subUrl, ua, userConfig) {
    ...
    userConfig.customAppGroups.forEach(cg => {
        if (cg.rules) {
            cg.rules.forEach(r => {
                // 无论用户导入时写什么目标，这里强制改为组名
                injectedRules.push(`${r.type},${r.value},${cg.name}${r.noResolve ? ',no-resolve' : ''}`);
            });
        }
    });
...
function renderAdminPage(config) {
    ...
    // 注意：所有客户端 JS 代码均使用单引号拼接，防止与外层模板冲突
    return `
<!DOCTYPE html>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="edit-batch">
                        <div class="alert alert-info small p-2">
                            <strong>智能导入引擎</strong>：支持直接粘贴 Clash 配置文件内容。
                            <ul>
                                <li>自动去除注释 (#) 和 YAML 列表符 (-)</li>
                                <li>自动忽略原有目标 (Proxy/Direct)，规则自动归入当前组</li>
                                <li>支持简写：DOMAINSUFFIX -> DOMAIN-SUFFIX</li>
                            </ul>
                        </div>
...
<div class="container" id="main-app" style="max-width:950px">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="fw-bold">🛠️ NextReia Pro V6.8</h3>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade show active" id="config-pane">
            <div class="card border-primary border-2">
                <div class="card-body d-flex justify-content-between align-items-center">
                    ...
                    <div class="form-check form-switch form-switch-lg"><input class="form-check-input" type="checkbox" role="switch" id="enable_overwrite" style="transform: scale(1.5);"></div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-check form-switch mb-3"><input class="form-check-input" type="checkbox" id="dns_enable"><label class="form-check-label fw-bold">启用</label></div>
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label small">IPv6</label><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="dns_ipv6"></div></div>
                        <div class="col-md-6"><label class="form-label small">Enhanced Mode</label><select id="dns_enhanced" class="form-select form-select-sm"><option value="fake-ip">fake-ip</option><option value="redir-host">redir-host</option></select></div>
                        <div class="col-md-6"><label class="form-label small">Fake-IP Range</label><input type="text" id="dns_fakeip" class="form-control form-control-sm"></div><div class="col-md-6"><label class="form-label small">Use Hosts</label><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="dns_hosts"></div></div>
                        <div class="col-12"><label class="form-label small">Default Nameserver</label><textarea id="dns_default_ns" class="form-control" rows="2"></textarea></div>
                        <div class="col-12"><label class="form-label small">Nameserver</label><textarea id="dns_ns" class="form-control" rows="3"></textarea></div>
                        <div class="col-12"><label class="form-label small">Fallback</label><textarea id="dns_fallback" class="form-control" rows="3"></textarea></div>
                        <div class="col-12"><hr><h6>Fallback Filter</h6></div><div class="col-md-4"><label class="form-label small">GeoIP</label><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="dns_geoip"></div></div><div class="col-md-8"><label class="form-label small">IP CIDR</label><textarea id="dns_ipcidr" class="form-control" rows="2"></textarea></div><div class="col-12"><label class="form-label small">Domain</label><textarea id="dns_domain" class="form-control" rows="3"></textarea></div>
                    </div>
                </div>
            </div>
        ...
        <div class="tab-pane fade" id="stats-pane">
            <div class="card"><div class="card-header bg-body-tertiary d-flex justify-content-between align-items-center"><span>📊 统计 <span id="total-req" class="badge bg-secondary ms-2"></span></span><div><div class="btn-group me-2"><button class="btn btn-sm btn-outline-primary active" id="btn-ua" onclick="switchStats('ua')">UA视图</button><button class="btn btn-sm btn-outline-primary" id="btn-ip" onclick="switchStats('ip')">IP视图</button></div><button class="btn btn-sm btn-outline-danger me-2" onclick="clearStats()">清空</button><button class="btn btn-sm btn-outline-secondary" onclick="loadStats()">刷新</button></div></div><div class="card-body"><div class="chart-container d-flex justify-content-center"><canvas id="statsChart"></canvas></div><div id="stats_tables"></div></div></div>
        </div>
    </div>
</div>
...
<script>
    ...
    function smartBatchImport() {
        ...
        lines.forEach(line => {
            // 1. Clean Line: remove comment, leading dash, trim
            let clean = line.split('#')[0].split('//')[0].trim();
            if(!clean) return;
            clean = clean.replace(/^-\\s*/, '').trim();
            
            // 2. Split parts by comma
            const parts = clean.split(',').map(s => s.trim());
            
            if(parts.length >= 2) {
                let type = parts[0].toUpperCase();
                let val = parts[1];
                let noResolve = clean.toLowerCase().includes('no-resolve');
                let target = '';
                ...
                if(ALL_RULE_TYPES.includes(type)) {
                    // Logic: If Global mode, try to keep target. If Group mode, ignore target (it becomes the group name).
                    if(editingMode === 'global') {
                        if(parts.length > 2 && !parts[2].toLowerCase().includes('no-resolve')) target = parts[2];
                    }
                    addRuleRow(type, val, target, noResolve);
                    count++;
                }
            }
        });
    ...
    function renderDNS() {
        ...
        const h = document.getElementById('dns_fields'); // Ensure this ID exists in HTML
        if(!h) return; // Safety
        ...
        document.getElementById('dns_fakeip').value = d['fake-ip-range'];
        document.getElementById('dns_hosts').checked = d['use-hosts'];
        document.getElementById('dns_default_ns').value = d['default-nameserver'].join('\\n');
        document.getElementById('dns_ns').value = d.nameserver.join('\\n');
        document.getElementById('dns_fallback').value = d.fallback.join('\\n');
        document.getElementById('dns_geoip').checked = d['fallback-filter'].geoip;
        document.getElementById('dns_ipcidr').value = d['fallback-filter'].ipcidr.join('\\n');
        document.getElementById('dns_domain').value = d['fallback-filter'].domain.join('\\n');
    }
