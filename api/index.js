 ...
 * Version: 4.1 (Fix Dark Mode & Chart Display)
 */
...
function renderAdminPage(config) {
    ...
    // 修复深色模式：不再使用内联样式强制背景色，而是使用 CSS 类和 Style 块动态生成
...
<head>
    ...
    <style>
        :root { --blur-amt: 12px; }
        body { background-color: var(--bs-body-bg); transition: background 0.3s; padding: 20px; min-height: 100vh; }
...
<div class="modal fade" id="ruleModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">编辑规则 - <span id="ruleModalTitle"></span></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="alert alert-info small py-2">规则将插入到订阅最前方。</div>
...
<div class="container" id="main-app" style="max-width:900px">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="config-pane">
            ...
            <div class="card">
                <div class="card-header text-warning bg-body-tertiary d-flex justify-content-between align-items-center"><span>2. 分流策略组 (支持排序/新增)</span><button class="btn btn-sm btn-success" onclick="addNewCustomGroup()">+ 新增自定义组</button></div>
                <div class="card-body"><div class="alert alert-secondary small py-1 mb-2">💡 长按拖拽图标排序。默认组不可删除/改名。</div><ul class="list-group" id="sortable-groups"></ul></div>
            </div>
        ...
        <div class="tab-pane fade" id="ui-pane">
            ...
            <div class="card border-danger"><div class="card-header text-danger">🧨 危险区域</div><div class="card-body"><button class="btn btn-danger w-100" onclick="factoryReset()">恢复出厂设置 (清除所有数据)</button></div></div>
        </div>

        <div class="tab-pane fade" id="stats-pane">
            <div class="card"><div class="card-header bg-body-tertiary d-flex justify-content-between align-items-center"><span>📊 24H 统计</span><div><button class="btn btn-sm btn-outline-danger me-2" onclick="clearStats()">清空</button><button class="btn btn-sm btn-outline-secondary" onclick="loadStats()">刷新</button></div></div><div class="card-body"><div class="chart-container d-flex justify-content-center"><canvas id="statsChart"></canvas></div><h6 class="mt-4 mb-3 border-bottom pb-2">客户端详情</h6><div id="stats_tables"></div></div></div>
        </div>
    </div>
</div>
...
<script>
    ...
    function openRuleEditor(groupName) {
        editingGroupName = groupName; document.getElementById('ruleModalTitle').innerText = groupName;
        const grp = config.customAppGroups.find(g => g.name === groupName);
        const rules = grp ? (grp.rules || []) : []; const targets = grp ? (grp.targetLBs || []) : [];
    ...
    function addRuleRow(type = 'DOMAIN-SUFFIX', val = '') {
        ...
        div.innerHTML = \`<select class="form-select form-select-sm rule-type" style="max-width: 150px;"><option value="DOMAIN-SUFFIX" \${type==='DOMAIN-SUFFIX'?'selected':''}>DOMAIN-SUFFIX</option><option value="DOMAIN-KEYWORD" \${type==='DOMAIN-KEYWORD'?'selected':''}>DOMAIN-KEYWORD</option><option value="IP-CIDR" \${type==='IP-CIDR'?'selected':''}>IP-CIDR</option><option value="SRC-IP-CIDR" \${type==='SRC-IP-CIDR'?'selected':''}>SRC-IP-CIDR</option></select><input type="text" class="form-control form-control-sm rule-value" placeholder="值" value="\${val}"><button class="btn btn-outline-danger btn-sm" onclick="this.parentElement.remove()">×</button>\`;
        document.getElementById('rule-list-container').appendChild(div);
    }
    function saveRulesFromModal() {
        if (!editingGroupName) return;
        const rows = document.querySelectorAll('.rule-row');
        const newRules = Array.from(rows).map(row => ({ type: row.querySelector('.rule-type').value, value: row.querySelector('.rule-value').value })).filter(r => r.value);
    ...
    function renderAppGroups() {
        ...
        config.groupOrder.forEach(appName => {
            ...
            let html = \`<div class="d-flex justify-content-between"><span class="fw-bold mb-1">\${appName} \${!isDefault ? '<small class="text-info">(自定义)</small>' : ''}</span></div><div class="checkbox-grid d-flex flex-wrap gap-2">\`;
            getLBNames().forEach(lb => {
    ...
    async function save() {
        ...
        const newConfig = { 
            ...config, lbGroups, appGroups, customAppGroups: updatedCustomGroups, groupOrder: config.groupOrder, dnsSettings, 
    ...
    function renderStats(data, isOverwriteEnabled) {
        ...
        const createTable = (title, items) => {
            ...
            let html = \`<h6 class="mt-4">\${title}</h6><div class="table-responsive"><table class="table table-sm table-striped"><thead><tr><th>UA</th><th>覆写</th><th class="text-end">次数</th></tr></thead><tbody>\`;
            items.forEach(i => {
        ...
        data.forEach(item => {
            ...
            if (/Clash|Mihomo/i.test(item.ua)) simple = "Clash/Mihomo";
            if (/Shadowrocket/i.test(item.ua)) simple = "Shadowrocket";
    ...
    async function factoryReset() { if(!confirm("⚠️ 危险：恢复出厂设置？")) return; await fetch('/?action=factoryReset', { method: 'POST' }); alert("已重置"); location.reload(); }
    async function resetConfig() { if(!confirm("重置配置？")) return; await fetch('/?action=resetConfig', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ authHash: authTokenHash }) }); alert("已重置配置"); location.reload(); }
